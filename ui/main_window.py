...
class App(ctk.CTk):
    ...
    def create_sidebar(self):
        ...
        if state.AppState.is_auto_mode: self.mode_switch.select()

        # Render trigger
        ctk.CTkLabel(self.sidebar_frame, text="⌨️ 渲染触发热键:", anchor="w", font=ctk.CTkFont(weight="bold")).pack(padx=20, pady=(15, 0), anchor="w")
        ...
        # Interaction Mode Options (将MD模式整合为选项4)
    ...
    def _build_menus(self):
        ...
        math_menu.add_command(label="矩阵 (matrix)", command=lambda: pyperclip.copy(r"$\begin{matrix} 1 & 0 \\ 0 & 1 \end{matrix}$"))
        ...
        # 使用字典映射：键为菜单显示的名称，值为 Markdown 识别的语言标签
        lang_map = {
            "Python": "python",
            "C": "c",
            "C#": "csharp",
    ...
    def create_main_panel(self):
        ...
        self.html_label = ctk.CTkLabel(self.main_frame, text="🏷️ 编译注入到 OneNote 的 HTML 结构", font=ctk.CTkFont(size=14, weight="bold"))
